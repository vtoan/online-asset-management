        ...
        public static FileContentResult ExportExcel(ICollection<ReportModel> report, string workSheetName, string fileName)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                for (int i = 0; i < properties.Length; i++)
                {
                    ...
                    worksheet.Cell(currentRow, i + 1).Value = displayName != null ? displayName.DisplayName : properties[i].Name;
                }
                foreach (var reportmodel in report)
                {
                    ...
                    {
                        worksheet.Cell(currentRow, i + 1).Value = propertyInfos[i].GetValue(reportmodel, null);
                    }
                }
